    ...
    try:
        ...
        for col in all_missing_cols:
            ...
            if ml_row[f'{col}_missing'].sum() > 0:
                logger.warning("Missing values found in column '%s'. Created indicator column '%s_missing'.", col, col)

        return ml_row
    except Exception:
        logger.exception("Failed to create missing value indicators for specific columns")
        raise
