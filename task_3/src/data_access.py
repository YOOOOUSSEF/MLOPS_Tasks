    ...
    try:
        ...
        if uri.startswith("models:/") and "@" in uri:
            try:
                ...
                logger.warning(
                    "Could not resolve alias locally (%s); falling back to direct URI load.",
                    resolve_err,
                )
...
def create_ML_TABLE(connection_string: str) -> pd.DataFrame:
    ...
    top_categories = (
        order_items_full["product_category"].value_counts().nlargest(20).index
    )
    order_items_full["product_category_grouped"] = order_items_full[
        "product_category"
    ].where(order_items_full["product_category"].isin(top_categories), "other")

    category_dummies = pd.get_dummies(
        order_items_full["product_category_grouped"], prefix="cat"
    )
    state_dummies = pd.get_dummies(order_items_full["seller_state"], prefix="sel_state")
    order_items_full = order_items_full.drop(
        columns=["product_category_grouped", "seller_state", "product_category"]
    )
    order_items_full = pd.concat(
        [order_items_full, category_dummies, state_dummies], axis=1
    )

    state_cols = [
        col for col in order_items_full.columns if col.startswith("sel_state_")
    ]
    ...
    payment_dummy_cols = [
        c for c in order_payments_full.columns if c.startswith("pay_")
    ]
    ...
    order_payments_agg = (
        order_payments_full.groupby("order_id").agg(**agg_dict).reset_index()
    )
    ...
    final_table = final_table.merge(
        customers_and_zipcodes, on="customer_id", how="left"
    )
...
def create_order_features(order_ids: list[str]) -> pd.DataFrame:
    ...
    try:
        ...
        orders = (
            orders.set_index("order_id").loc[order_ids].reset_index()
        )  # to restore order.
        ...
        state_dummies = pd.get_dummies(
            order_items_full["seller_state"], prefix="sel_state"
        )
        order_items_full = order_items_full.drop(
            columns=["product_category_grouped", "seller_state", "product_category"]
        )
        ...
        category_cols = [
            col for col in order_items_full.columns if col.startswith("cat_")
        ]
        agg_dict = {
            "num_sellers": ("seller_id", "nunique"),
            "num_items": ("order_item_id", "count"),
        ...
        order_items_agg = (
            order_items_full.groupby("order_id").agg(**agg_dict).reset_index()
        )
        payment_dummies = pd.get_dummies(order_payments["payment_type"], prefix="pay")
        order_payments_full = pd.concat([order_payments, payment_dummies], axis=1)
        ...
        logger.info(
            "Order feature creation completed for order_id=%s", ", ".join(order_ids)
        )
        return final_row
    except Exception:
        logger.exception(
            "Failed to build order features for order_id=%s", ", ".join(order_ids)
        )
        raise
